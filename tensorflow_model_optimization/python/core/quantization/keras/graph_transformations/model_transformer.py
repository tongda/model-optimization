...
class ModelTransformer(object):
  ...
  def _is_supported_given_num_inputs_outputs(self, layer, is_head_node):
    """Check if ModelTransformer supports transformations given input and output info at a layer."""
    assert self._is_functional_model(self.model)
  ...
  def _get_input_layer_names(self, layer):
    ...
    if self._is_functional_model(self.model):
      ...
      if i == -1:
        raise ValueError('Layer not found in  model configuration.')
      elif i == 0:
  ...
  def _match_layer_with_inputs(self, layer, pattern, is_head_node):
    ...
    if self._is_functional_model(self.model):
      if not self._is_supported_given_num_inputs_outputs(layer, is_head_node):
        return None
  ...
  def _replace_sequential(self, match_layer_node, replacement_layer_node):
    ...
    def _get_replacement_nodes(head_replacement_node):
      """Get list of replacement nodes in Sequential order."""
      replacement_nodes = []

      def _get_replacement_nodes_recurse(replacement_node):
        assert len(replacement_node.input_layers) <= 1

        for input_layer in replacement_node.input_layers:
          _get_replacement_nodes_recurse(input_layer)
        replacement_nodes.append(replacement_node)

      _get_replacement_nodes_recurse(head_replacement_node)
